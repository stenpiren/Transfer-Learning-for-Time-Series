...
'''RNNを一層のみにする場合、return_sequences=Falseにする'''
stop = EarlyStopping(patience=10,verbose=1)
def create_model():
    model = Sequential()
    model.add(RNN(input_shape=(X_train_time.shape[1], X_train_time.shape[2]), units = 50, return_sequences=True))
...
condition = "1000s_50u_2n_100epc"
if not os.path.exists("./{}/RNN".format(condition)):
    os.makedirs("{}/RNN".format(condition))
...
def plot(num, history):
    ...
    plt.savefig('{}/RNN/plot{}.png'.format(condition, num))
...
plt.ylim(0,1)
plt.legend(loc="best")
plt.savefig('{}/RNN/test1.png'.format(condition))
...
plt.ylim(0,1)
plt.legend(loc="best")
plt.savefig('{}/RNN/test2.png'.format(condition))

with open("{}/RNN/result_info.txt".format(condition),"w") as f:
