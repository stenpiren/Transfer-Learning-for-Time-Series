...
def create_model():
    model = Sequential([
        GRU(input_shape=(X_train_time.shape[1], X_train_time.shape[2]), units = 50, return_sequences=True),
...
condition = "nonormalize_cu_lin_1000s_50u_2n_100epc"
...
plt.figure(figsize=(18,5))
plt.plot([ i for i in range(1, 1+len(predict1))], predict2, 'r',label="predicted")
plt.plot([ i for i in range(1, 1+len(predict1))], y2_test_time, 'b',label="measured", lw=1, alpha=0.3)
plt.legend(loc="best")
plt.xlim(0,1000)
plt.ylim(0,1)
plt.xlabel("time / min")
