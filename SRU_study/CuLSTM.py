...
'''LSTMを一層のみにする場合、return_sequences=Falseにする'''

stop = EarlyStopping(patience=10,verbose=1)
def create_model():
    model = Sequential([
        LSTM(input_shape=(X_train_time.shape[1], X_train_time.shape[2]), units = 50, return_sequences=True),
        ...
        # model.add(Dropout(0.3))
        Dense(1, activation='linear', activity_regularizer=regularizers.l2(0.01)),
    ])
...
t1 = time()
model1 = create_model()
history1 = model1.fit(X_train_time, y1_train_time, epochs=100, batch_size=500, validation_data=(X_valid_time, y1_valid_time), verbose=1, shuffle=False, callbacks=[stop])
model2 = create_model()
history2 = model2.fit(X_train_time, y2_train_time, epochs=100, batch_size=500, validation_data=(X_valid_time, y2_valid_time), verbose=1, shuffle=False, callbacks=[stop])
t2 = time()
history = {"1":history1, "2":history2}
condition = "nonormalize_cu_sig_1000s_50u_2n_100epc"
...
def plot(num, history):
    ...
    plt.tight_layout()
    plt.savefig('{}/LSTM/plot{}.png'.format(condition, num))
...
plt.ylabel("value / -")
plt.tight_layout()
plt.savefig('{}/LSTM/test1.png'.format(condition))

predict2 = model2.predict(X_test_time).reshape(-1)
plt.figure(figsize=(18,5))
plt.plot([ i for i in range(1, 1+len(predict2))], predict2, 'r',label="predicted")
plt.plot([ i for i in range(1, 1+len(predict2))], y2_test_time, 'b',label="measured", lw=1, alpha=0.3)
plt.legend(loc="best")
plt.xlim(0,1000)
plt.ylim(0,1)
plt.xlabel("time / min")
plt.ylabel("value / -")
plt.tight_layout()
plt.savefig('{}/LSTM/test2.png'.format(condition))

with open("{}/LSTM/result_info.txt".format(condition),"w") as f:
